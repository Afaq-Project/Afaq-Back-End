# Batch 2 — Endpoint Specification

**Feature**: Profile Module v2 Redesign
**Batch**: 02 — Education Section
**Base URL**: `/api/v1`
...
**Completion Weight Impact**: Adds up to **35%** to `completionPct` (10% for `educationLevelId` + 25% for at least one education record).
...
## 1. `POST /api/v1/profile/educations`
...
## 2. `GET /api/v1/profile/educations`
...
## 3. `GET /api/v1/profile/educations/:id`
...
## 4. `PATCH /api/v1/profile/educations/:id`
...
## 5. `DELETE /api/v1/profile/educations/:id`
...
## 6. `GET /api/v1/reference/major-categories` [PUBLIC]
...
## 7. `GET /api/v1/reference/majors` [PUBLIC]
...
## 8. `GET /api/v1/reference/institutions` [PUBLIC]
...
| `201` | Created | Successful education record creation |
| `204` | No Content | Successful deletion |
| `400` | Bad Request | Validation failure, invalid FK, business rule violation |
...
Every successful mutation on `/profile/educations/*` triggers `ProfileService.recalculate(userId)` **before** the response is returned. The response reflects the freshly computed `completionPct`, `isMatchable`, and `matchingVersion`.

### `completionPct` Impact Summary

| Action | `completionPct` Change |
|:---|:---|
| Add `educationLevelId` via `PATCH /profile/personal` | +10 |
| Add first education record via `POST /profile/educations` | +25 |
| Add additional education records | 0 (no further increase) |
| Delete the only education record | −25 |
| Modify an education record (GPA, dates) | 0 (already counted) |

### Swagger Requirements

Every endpoint must document:
- `@ApiTags('Profile — Educations')` or `@ApiTags('Reference')`
- `@ApiOperation` with summary and description
- `@ApiResponse` for `200`/`201`/`204` and each error code
- `@ApiBearerAuth` (except public endpoints, which use `@Public()`)
- `@ApiQuery` for each query parameter on reference endpoints
